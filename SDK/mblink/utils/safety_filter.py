...
class SafetyFilter(BasePolicy):
  ...
  def __init__(
      self, base_policy: BasePolicy, safety_policy: BasePolicy, monitor: BaseMonitor, override_type: str = 'naive',
      concise: bool = False
  ):
  ...
  def override(self, obs: np.ndarray, agents_action: Dict[str, np.ndarray], monitor_info: dict, **kwargs):
    if self.override_type == 'naive':
      a_s, safety_info = self.safety_policy.get_action(
          obs=obs, agents_action=agents_action, monitor_info=monitor_info, **kwargs
      )
    elif self.override_type == 'linear':
      has_linear_policy = ((self.time_idx is not None)
      ...
      if has_linear_policy:
        ...
        a_s, safety_info = self.linear_policy.get_action(
            obs=obs, agents_action=agents_action, monitor_info=monitor_info, **kwargs_dup
        )
        self.time_idx += 1
      else:
        a_s, safety_info = self.safety_policy.get_action(
  ...
  def get_action(self, obs: np.ndarray, agents_action: Optional[Dict[str, np.ndarray]] = None,
    ...
    override_flag, monitor_info = self.monitor.check(obs=obs, agents_action=action_dict, **kwargs)
    ...
    if override_flag:
      a_s, safety_info = self.override(obs, action_dict, monitor_info, **kwargs)
      info['shield'] = True
