...
def get_box_samples(bound: np.ndarray, n_pts_per_axis: List[Optional[int]]):
  ...
  if len(n_pts_per_axis) != bound.shape[0]:
    raise ValueError(f"The length of n_pts_per_axis should be {bound.shape[0]}.")
  gridpoints = []
...
def get_zono_samples_mixed(zono: Zonotope, vert_axes: List[int], box_axes: List[int],
                           eps: float) -> Tuple[np.ndarray, int]:
...
def get_zonotope_reachset(
    init_box: np.ndarray, a_mat_list: List[np.ndarray], b_mat_list: List[np.ndarray], input_box_list: List[np.ndarray],
    bias_list: Optional[List[np.ndarray]] = None, save_list: Optional[List[bool]] = None
) -> List[Zonotope]:
  ...
  assert len(a_mat_list) == len(b_mat_list) == len(input_box_list), ("all lists should be same length")
  ...
  iterate_zonotope_reachset(init_box, a_mat_list, b_mat_list, input_box_list, bias_list, custom_func)
...
def iterate_zonotope_reachset(
  ...
  for a_mat, b_mat, input_box, bias in zip(a_mat_list, b_mat_list, input_box_list, bias_list):
    ...
    if b_mat is not None:
      ...
      assert len(
          z.init_bounds
      ) == num_gens, (f"Zonotope had {num_gens} generators, " + f"but only {len(z.init_bounds)} bounds were there.")
...
# == CHECK ==
def check_verts_in_box(verts: np.ndarray, center: float, box: np.ndarray, axis: int) -> bool:
  """
  Args:
