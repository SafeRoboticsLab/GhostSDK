...
def get_values(
    ...
    value_fn: Callable[[np.ndarray, Optional[Union[np.ndarray, torch.Tensor]]], np.ndarray], xs: np.ndarray,
    ys: np.ndarray, v: float, yaw: float, batch_size: int, delta: float = 0., fail_value: float = -1.0
):
  values = np.full((xs.shape[0], ys.shape[0]), fill_value=fail_value)
  it = np.nditer(values, flags=['multi_index'])
  while not it.finished:
    idx_all = []
    obs_all = np.empty((0, env.obsrv_dim), dtype=float)
    while len(idx_all) < batch_size:
      ...
      g_x = -env.constraint.get_cost(state_jnp, control_jnp, closest_pt, slope, theta, time_indices=dummy_time_indices)
      g_x = np.asarray(g_x)[0]
      if g_x > 0.:  # These sates are not in the failure set.
        obsrv = env.get_obsrv(state.reshape(-1))
        obs_all = np.concatenate((obs_all, obsrv.reshape(1, -1)))
        idx_all += [idx]
      it.iternext()
      if it.finished:
        break
    v_all = value_fn(obs_all, append=None)
...
def plot_traj(
    ax, trajectory: np.ndarray, result: int, c: str = 'b', lw: float = 2., zorder: int = 1, vel_scatter: bool = False,
    s: int = 40
):
  traj_x = trajectory[:, 0]
  traj_y = trajectory[:, 1]
  ...
  if vel_scatter:
    vel = trajectory[:, 2]
    ax.scatter(
        traj_x[0], traj_y[0], s=s, c=vel[0], cmap=cm.copper, vmin=0, vmax=2., edgecolor='none', marker='s',
        zorder=zorder
    )
    ax.scatter(
        traj_x[1:-1], traj_y[1:-1], s=s - 12, c=vel[1:-1], cmap=cm.copper, vmin=0, vmax=2., edgecolor='none',
    ...
    if result == -1:
      ...
      edgecolor_final = 'y'
    ax.scatter(
        traj_x[-1], traj_y[-1], s=s, c=vel[-1], cmap=cm.copper, vmin=0, vmax=2., edgecolor=edgecolor_final,
        marker=marker_final, zorder=zorder
    )
  else:
...
def plot_poly(
    ax, poly: Polygon, c: str = 'g', alpha: float = 1., lw: float = 1., plot_box: bool = False, c_box: str = 'm',
    alpha_box: float = 1., lw_box: float = 1., zorder: int = 1
):
  x, y = poly.exterior.xy
  ax.plot(x, y, c=c, alpha=alpha, lw=lw, zorder=zorder)
  if plot_box:
    xmin, ymin, xmax, ymax = poly.bounds
    ax.plot([xmin, xmax, xmax, xmin, xmin], [ymin, ymin, ymax, ymax, ymin], c=c_box, alpha=alpha_box, lw=lw_box,
            zorder=zorder)


def plot_poly_with_fp_single_yaw(
    ax, frs_poly: Polygon, env: RaceCarDstb5DEnv, state: np.ndarray, c: str = 'orange', alpha: float = 1.,
    lw: float = 1
) -> Polygon:
...
def plot_poly_with_fp(
    ax, frs_verts: np.ndarray, env: RaceCarDstb5DEnv, state: np.ndarray, c: str = 'orange', alpha: float = 1.,
    lw: float = 1, zorder: int = 1
) -> Polygon:
  x_all = np.empty((0,))
  y_all = np.empty((0,))
...
def plot_step(
  ...
  if 'frs' in solver_info['monitor']:
    ...
    ax.scatter(traj_n[1, 0], traj_n[1, 1], c=color_dict['nominal'], s=s, zorder=2)

    if plot_even_failed:
      ax.plot(traj_n[stop - 1:, 0], traj_n[stop - 1:, 1], linewidth=2., c='b', alpha=0.5)

    if plot_linear:
      env.agent.policy = monitor_info['frs']['linear_policy']
      traj_l, _, _ = env.simulate_one_trajectory(
          T_rollout=traj_n.shape[0] - 1, end_criterion='timeout', adversary=test_adversary,
          reset_kwargs={'state': traj_n[0].copy()}
      )
      ax.plot(traj_l[:stop, 0], traj_l[:stop, 1], linewidth=2, c=color_dict['linear'], linestyle='--')
      if plot_even_failed:
        ax.plot(
            traj_l[stop - 1:, 0], traj_l[stop - 1:, 1], linewidth=2, c=color_dict['linear'], linestyle='--', alpha=0.5
        )
    ...
    for i, zono in enumerate(zonotopes):
      ...
      if plot_footprint:
        ...
        plot_poly_with_fp(ax, verts, env, state=traj_n[i], c=color_dict['fp'], alpha=alpha_footprint, lw=1.)
    ...
    env.render_footprint(ax, state=traj_n[-1], c=color_dict['nominal'], lw=1., alpha=1.)
  ...
  if only_check:
    ...
    if solver_info['shield']:
      if 'frs' in solver_info['monitor']:
        ...
        txt = ''
      ax.set_title(f"Shields due to {monitor_info['raised_reason']}" + txt, fontsize=fontsize)
    else:
      ax.set_title("Uses task ctrl", fontsize=fontsize)
    ax.legend(ncol=3, framealpha=0., loc=10, bbox_to_anchor=(0.5, -0.2), fontsize=fontsize - 4)
  plt.tight_layout()
  if savefig:
