...
class TwinnedQNetwork(nn.Module):
  ...
  def __init__(
      self, obs_dim, mlp_dim, action_dim, append_dim=0, latent_dim=0,
...
class GaussianPolicy(nn.Module):
  ...
  def __init__(
      self, obs_dim, mlp_dim, action_dim, action_range, append_dim=0,
    ...
    if isinstance(action_range, np.ndarray):
      ...
      action_range = torch.FloatTensor(np.array(action_range)).to(self.device)
